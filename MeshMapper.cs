...
namespace MeshCentralRouterCmd
{
    public class MeshMapper
    {
        ...
        public void Debug(string msg) {
            ...
            if (xdebug) { try { File.AppendAllText("debug.log", "Debug-MeshMapper-" + msg + "\r\n"); } catch (Exception) { } }
        }
        ...
        private void UpdateInfo() {
            ...
            if (state == -1) { msg = "Unable to bind to local port"; }
                ...
                if (remoteip == null)
                {
                    msg = string.Format("Mapped local port {0} to remote port {1}", localport, remoteport);
                } else {
                    msg = string.Format("Mapped local port {0} to remote {1}:{2}", localport, remoteip, remoteport);
                }
        ...
        {
            Debug("#" + sender.id + ": Websocket mapping connection to server: " + state.ToString());
            switch (state)
            {
        ...
        private void Wc_onBinaryData(webSocketClient sender, byte[] data, int offset, int length, int orglen)
        {
            ...
            if (sender.tag.GetType() == typeof(TcpClient))
            {
                ...
                if (client != null) {
                    ...
                    Debug("#" + sender.id + ": Remote --> Local, " + length);
        ...
        private void ClientEndReadWS(IAsyncResult ar)
        {
            ...
            if (len > 0)
            {
                ...
                try
                {
                    ...
                    Debug("#" + counter + ": Local --> Remote, " + len);
