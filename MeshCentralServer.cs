...
namespace MeshCentralRouterCmd
{
    ...
    public class MeshCentralServer
    {
        ...
        private static RandomNumberGenerator cryptoRandom = RandomNumberGenerator.Create();
        ...
        */
        ...
        public void connect(Uri wsurl, string user, string pass, string token)
        {
            ...
            {
                // Request inner authentication
                extraHeaders.Add("x-meshauth", "*");
            }
                ...
                case "serverAuth":
                    {
                        ...
                        byte[] certbin = System.Convert.FromBase64String((string)jsonAction["cert"]);
                        byte[] nonce = System.Convert.FromBase64String((string)jsonAction["nonce"]);
                        byte[] signature = System.Convert.FromBase64String((string)jsonAction["signature"]);
                        X509Certificate2 cert = new X509Certificate2(certbin);
                        string certhash = webSocketClient.GetMeshKeyHash(cert);
                        if (certhash != serverid) { disconnectMsg = "InvalidServerId"; disconnect(); return; }

                        // Hash the signed data and verify the server signature
                        byte[] concat = new byte[144];
                        clientNonce.CopyTo(concat, 0);
                        StringToByteArray(certHash).CopyTo(concat, 48);
                        nonce.CopyTo(concat, 96);
                        var rsaRead = new RSACryptoServiceProvider();
                        rsaRead.FromXmlString(cert.GetRSAPublicKey().ToXmlString(false));
                        if (rsaRead.VerifyData(concat, new SHA384CryptoServiceProvider(), signature) == false) { disconnectMsg = "InvalidServerSignature"; disconnect(); return; }

                        // Authenticate the server using HTTPS cert hash
                        if (token != null)
                        {
                            wc.SendString("{\"action\":\"userAuth\",\"username\":\"" + Base64Encode(user) + "\",\"password\":\"" + Base64Encode(pass) + "\",\"token\":\"" + token + "\"}");
                        }
                        else
                        {
                            wc.SendString("{\"action\":\"userAuth\",\"username\":\"" + Base64Encode(user) + "\",\"password\":\"" + Base64Encode(pass) + "\"}");
                        }
                        break;
                    }
                case "serverinfo":
        ...
        private void changeStateEx(webSocketClient sender, webSocketClient.ConnectionStates newState)
        {
            ...
            if (newState == webSocketClient.ConnectionStates.Connected) {
                if (serverid != null)
                {
                    clientNonce = new byte[48];
                    cryptoRandom.GetBytes(clientNonce);
                    string clientNonceb64 = System.Convert.ToBase64String(clientNonce);
                    wc.SendString("{\"action\":\"serverAuth\",\"cnonce\":\"" + clientNonceb64 + "\",\"tlshash\":\"" + certHash + "\"}"); // Ask for server authentication
                }
            }
        }
    ...
    public class NodeClass
    {
        ...
        {
            string status = "";
            if (mtype == 3) return "Local";
            if ((conn & 1) != 0) { if (status.Length > 0) { status += ", "; } status += "Agent"; }
            if ((conn & 2) != 0) { if (status.Length > 0) { status += ", "; } status += "CIRA"; }
            if ((conn & 4) != 0) { if (status.Length > 0) { status += ", "; } status += "AMT"; }
            if ((conn & 8) != 0) { if (status.Length > 0) { status += ", "; } status += "Relay"; }
            if ((conn & 16) != 0) { if (status.Length > 0) { status += ", "; } status += "MQTT"; }
